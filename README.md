...
Первый прогон по X-кабелю (2026-09-22): рукопожатие, `.ping` и сетка терминала прошли. Снимок 24×80 часто не влезает в кадр 240 байт — в REPL `.resize 12 40`. Установщик на флешку кладёт Linux python3 и `alsa-utils` (без apt на сервере). SSH и IP поверх звука специально не делаются.
