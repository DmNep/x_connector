...
**1.0.0** — первая сборка принята (2026-09-25 на живом X-кабеле). Это релиз, не бета: консоль починки сети по звуку работает без консоли сервера.
...
- Клиент на Windows и агент на Linux — один пакет `xconn_channel` (Python, только стандартная библиотека).
- Ввод как в консоль: строка команды, клавиша (Ctrl‑C, стрелки, Tab) или файл в обе стороны (`.put`/`.get` — кадры `FILE_*`, CRC-32).
- Обратно — снимок VT100-сетки 24×80 (cp437), целиком, дельтой или кусками `SCREEN_PART`. Консоль ждёт окончания команды и глотает OSC.
- Полудуплекс с CRC и повтором кадра. Рабочий режим — `base` 1200 бод, тона Bell 202. Шифрования нет: оператор стоит у машины.
- Установка на сервер — с USB-флешки: `py -m xconn_channel stick E:\` пишет автономный установщик, `sudo sh install.sh` на сервере без интернета.
- Проверка без кабеля: `loopback` гоняет оба конца в одном процессе.

Приёмка первой сборки: после одной установки с флешки работа шла только по X-кабелю (`ip`, `ss`, netplan, `journalctl`, sudo). SSH и IP поверх звука не делаются — они требуют тот самый сетевой стек, который в этот момент сломан.

## Что не входит в 1.0.0

Это следующие фазы, не недоделки релиза:

- Формальный отчёт BER/FER по живому кабелю (`py tools/ber.py --live` уже есть; `--selftest` без железа зелёный).
- Режим `stretch` 2400 бод — выключен, пока нет этого отчёта и явного согласия включить.
- HID-клавиатура и слепой подъём канала.
- Шифрование, полный дуплекс, SSH/IP поверх звука — сознательный отказ, не «ещё не сделано».
...
На этом ноутбуке: `--capture 0 --playback 1` (карта 0 — HDMI, в неё играть бессмысленно). Агент на `termv100` — `plughw:1,0`.
...
Подробности — в [мануале](docs/manual.md).
