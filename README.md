...
**Первый прогон по X-кабелю состоялся (2026-09-22):** рукопожатие, `.ping` и сетка терминала прошли на реальном железе. С 0.4.0 несжимаемый 24×80 едет кусками `SCREEN_PART`, повторный HELO пробует и `probe`, и `base` (агент, оставшийся в `base` с прошлой сессии, больше не глушит новый клиент). SSH и IP поверх звука по-прежнему не делаются — они требуют тот самый сетевой стек, который в этот момент сломан.
