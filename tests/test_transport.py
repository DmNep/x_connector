...
class TestTransportRoundtrip(unittest.TestCase):
    ...
    def test_send_timing_fresh_request_has_no_lead(self) -> None:
        """Свежий REQ (после собственного GAP, а не после приёма) без T_LEAD (8.1, 8.4).

        Таблица 8.4 считает T_LEAD ровно один раз за обмен — перед
        ответом на принятый кадр, не перед инициирующим REQ.
        """
        link = SampleLink()
        sink_a, source_a = link.end_a()
        captured: list = []
        ...
        self.assertEqual(len(captured), 2, "без T_LEAD: только кадр и GAP")
        gap = round(config.GAP_MS * config.SAMPLE_RATE / 1000)
        self.assertEqual(bytes(captured[-1][-gap:]), b"\x00\x00" * gap)

    def test_send_timing_reply_has_lead_and_gap(self) -> None:
        """Ответ на только что принятый кадр окружён T_LEAD и GAP (docs/protocol.md 8.3, 8.4)."""
        link = SampleLink()
        sink_a, source_a = link.end_a()
        sink_b, source_b = link.end_b()
        captured: list = []
        ...
        ta = AudioTransport(capturing_sink, source_a, config.BASE)
        tb = AudioTransport(sink_b, source_b, config.BASE)

        req = framing.build_frame(config.PING, 0)
        tb.send(req)
        received = ta.receive(2.0)
        self.assertEqual(received, req, "предпосылка теста: кадр действительно принят")
        captured.clear()  # интересует только send() после приёма

        ta.send(framing.build_frame(config.PONG, 0))
    ...
    def test_idle_reset_recovers_next_frame(self) -> None:
        ...
        # Тишина на тракте: гейт должен реально увидеть отсутствие сигнала
        # (не просто хвост энергии в скользящем окне), прежде чем T_IDLE
        # сможет сбросить демодулятор (8.3) — на реальном канале это даёт
        # GAP отправителя, здесь эмулируем явно.
        tb._feed(mod.silence(200))
        tb._last_signal = 0.0
